...
def process_audio_with_white_noise(
    input_path: str,
    output_path: str,
...
class AudioPlayerWindow(QMainWindow):
    ...
    def setup_ui(self) -> None:
        ...
        self.noise_btn.setStyleSheet(
            "QPushButton { background-color: #cccccc; color: #666666; }"
        )
        ...
        self.status_label.setStyleSheet(
            "padding: 5px; background-color: #e0e0e0; border-radius: 3px;"
        )
    ...
    def select_annotation_file(self) -> None:
        ...
        Обрабатывает возможные ошибки при загрузке файла аннотации.
        """
        ...
        if file_path:
            try:
                ...
                if self.audio_paths:
                    for btn in [
                        self.prev_btn, self.play_btn, self.next_btn, self.noise_btn
                    ]:
                        btn.setEnabled(True)
                    self.noise_btn.setStyleSheet(
                    ...
                    self.status_label.setText(
                        f"Загружено {len(self.audio_paths)} аудиофайлов"
                    )
    ...
    def add_white_noise_to_current(self) -> None:
        ...
        try:
            ...
            success = process_audio_with_white_noise(
            ...
            if success:
                ...
                self.track_name_label.setText(
                    f"Трек {self.current_index + 1}: {track_name} (с шумом)"
                )

                QMessageBox.information(
                    self,
                    "Успех",
                    f"Аудио с белым шумом сохранено как:\n{noisy_filename}"
                )
            else:
    ...
    def playback_state_changed(self, state: QMediaPlayer.PlaybackState) -> None:
        ...
        """
        if (state == self.media_player.PlaybackState.StoppedState and
                self.is_playing):
            self.is_playing = False
            self.play_btn.setText("Воспроизвести")
    ...
    def slider_released(self) -> None:
        ...
        if self.media_player.duration() > 0:
            self.media_player.setPosition(
                int((position / 100) * self.media_player.duration())
            )
        if self.was_playing:
            self.media_player.play()
